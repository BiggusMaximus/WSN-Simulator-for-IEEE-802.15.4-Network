...
$$
E_\mathrm{sensing} = \left(V_s \, I_{\mathrm{sensing}} + P_\mathrm{MS}\right) \, T_\mathrm{sensing} \tag{1}
$$

$$
E_\mathrm{logging} = \frac{k_d \cdot {V_s}}{8}\left( I_\mathrm{write} \cdot T_\mathrm{write} +
                        I_\mathrm{read} \cdot T_\mathrm{read} \right) + P_\mathrm{MS} \cdot \left(T_\mathrm{read} + T_\mathrm{write}\right) \tag{2}
$$
